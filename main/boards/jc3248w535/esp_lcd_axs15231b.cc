                                      ...
                                      esp_lcd_panel_handle_t *ret_panel)
{
    ...
    {
        case LCD_RGB_ELEMENT_ORDER_RGB: axs15231b->madctl_val = 0; break;
        case LCD_RGB_ELEMENT_ORDER_BGR: axs15231b->madctl_val |= LCD_CMD_BGR_BIT; break;
    ...
    switch ( panel_dev_config->bits_per_pixel )
    {
        ...
        case 18: // RGB666
            ...
            // each color component (R/G/B) should occupy the 6 high bits of a byte, which means 3 full bytes are
            // required for a pixel
...
static esp_err_t rx_param(axs15231b_panel_t *axs15231b, esp_lcd_panel_io_handle_t io, int lcd_cmd, void *param,
                          size_t param_size)
{
...
static const axs15231b_lcd_init_cmd_t vendor_specific_init_default[] = {
    ...
    {AXS_LCD_SLPOUT, (uint8_t[]){0x00}, 0, 120},
    // {AXS_LCD_PTLON, (uint8_t[]){0x00}, 0, 0},
    {AXS_LCD_ALLPOFF, (uint8_t[]){0x00}, 0, 200},
};
...
static esp_err_t panel_axs15231b_init(esp_lcd_panel_t *panel)
{
    ...
    ESP_RETURN_ON_ERROR(tx_param(axs15231b, io, LCD_CMD_MADCTL,
                                 (uint8_t[]){
                                     axs15231b->madctl_val,
                                 },
                                 1),
                        TAG, "send command failed");
    ESP_RETURN_ON_ERROR(tx_param(axs15231b, io, LCD_CMD_COLMOD,
                                 (uint8_t[]){
    ...
    for ( int i = 0; i < init_cmds_size; i++ )
    {
        ...
        switch ( init_cmds[i].cmd )
        {
            ...
            case LCD_CMD_COLMOD:
                ...
                break;
            default: is_cmd_overwritten = false; break;
        }
        ...
        ESP_RETURN_ON_ERROR(tx_param(axs15231b, io, init_cmds[i].cmd, init_cmds[i].data, init_cmds[i].data_bytes), TAG,
                            "send command failed");
...
static esp_err_t panel_axs15231b_mirror(esp_lcd_panel_t *panel, bool mirror_x, bool mirror_y)
{
    ...
    tx_param(axs15231b, io, LCD_CMD_MADCTL, (uint8_t[]){axs15231b->madctl_val}, 1);
    return ESP_OK;
}
                                          ...
                                          esp_lcd_touch_handle_t *tp)
{
    ...
    esp_err_t ret = ESP_OK;
    esp_lcd_touch_handle_t axs15231b = (esp_lcd_touch_t *)calloc(1, sizeof(esp_lcd_touch_t));
